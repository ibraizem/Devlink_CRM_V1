...
export class MakeConnector extends BaseIntegrationConnector {
  ...
  async triggerScenario(data: Record<string, any>, webhookUrl?: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async sendCustomEvent(eventName: string, eventData: Record<string, any>): Promise<{ success: boolean; error?: string }> {
    return this.triggerScenario({
      type: 'custom_event',
      event: eventName,
      data: eventData,
